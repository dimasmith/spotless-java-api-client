...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { SpotlessInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw SpotlessInvalidDataException("`$name` is not set")
            is JsonNull -> throw SpotlessInvalidDataException("`$name` is null")
            else -> throw SpotlessInvalidDataException("`$name` is invalid, received $this")
        }
